...
def _get_all(doctype, filters=None, fields=None):
	if doctype == "Website Item":
		return [frappe._dict(item_code="BOARD", website_image="/files/board.jpg"), frappe._dict(item_code="BARE", website_image=None)]
	if doctype == "Item":
...
class TestVariantImage(unittest.TestCase):
	def test_a_variant_without_a_picture_shows_its_template_s(self):
		rows = [
			frappe._dict(item_code="BOARD-153", variant_of="BOARD", website_image=None),
			frappe._dict(item_code="BOARD-158", variant_of="BOARD", website_image="/files/own.jpg"),
			frappe._dict(item_code="BARE-1", variant_of="BARE", website_image=None),
			frappe._dict(item_code="BOARD", variant_of=None, website_image=None),
		]
		...
		self.assertEqual(rows[0].website_image, "/files/board.jpg")
	...
	def test_nothing_to_inherit_costs_no_query(self):
		rows = [frappe._dict(item_code="BOARD", variant_of=None, website_image=None)]
